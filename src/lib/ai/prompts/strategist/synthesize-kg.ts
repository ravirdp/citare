import type { RawClientData } from "@/lib/knowledge-graph/types";

/**
 * Build the strategist prompt for synthesizing a knowledge graph
 * from raw Google data sources.
 */
export function buildSynthesizeKGPrompt(rawData: RawClientData): string {
  const sourceSections: string[] = [];

  if (rawData.sources.googleAds) {
    sourceSections.push(
      `## Google Ads Data\n\`\`\`json\n${JSON.stringify(rawData.sources.googleAds, null, 2)}\n\`\`\``
    );
  }

  if (rawData.sources.gbp) {
    sourceSections.push(
      `## Google Business Profile Data\n\`\`\`json\n${JSON.stringify(rawData.sources.gbp, null, 2)}\n\`\`\``
    );
  }

  if (rawData.sources.searchConsole) {
    sourceSections.push(
      `## Google Search Console Data\n\`\`\`json\n${JSON.stringify(rawData.sources.searchConsole, null, 2)}\n\`\`\``
    );
  }

  if (rawData.sources.analytics) {
    sourceSections.push(
      `## Google Analytics Data\n\`\`\`json\n${JSON.stringify(rawData.sources.analytics, null, 2)}\n\`\`\``
    );
  }

  return `# Knowledge Graph Synthesis — Tier One Strategist
...
## Client Information
- **Client ID**: ${rawData.clientId}
- **Business Name**: ${rawData.clientName}
- **Business Type**: ${rawData.businessType}
- **Landmark Description**: ${rawData.landmarkDescription || "Not provided"}
